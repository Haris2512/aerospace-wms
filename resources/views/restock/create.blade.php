            ...
            <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="mb-8 flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-gray-400 text-sm mt-2">Request new stock from registered suppliers.</p>
                    </div>
                ...
                <div class="bg-[#151B2D] shadow-2xl sm:rounded-2xl overflow-hidden border border-[#2D3748]">
                    <div class="p-8">
                        ...
                        <form method="POST" action="{{ route('restock.store') }}">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                                ...
                                <div>
                                    ...
                                    <input id="order_date" type="date" name="order_date"
                                        ...
                                        class="block mt-2 w-full rounded-lg border-[#4A5568] bg-[#2D3748] text-white focus:border-blue-500 focus:ring-blue-500 shadow-sm py-2.5"
                                        required />
                                    @error('order_date') <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input id="expected_delivery_date" type="date" name="expected_delivery_date"
                                        ...
                                        class="block mt-2 w-full rounded-lg border-[#4A5568] bg-[#2D3748] text-white focus:border-blue-500 focus:ring-blue-500 shadow-sm py-2.5" />
                                    @error('expected_delivery_date') <p class="text-red-400 text-xs mt-1">{{ $message }}
                                    </p> @enderror
                                </div>
                            </div>
                            ...
                            {{-- BAGIAN 2: DAFTAR BARANG (DYNAMIC) --}}
                            ...
                            <div class="bg-[#1A202C] rounded-xl border border-[#2D3748] overflow-hidden mb-6">
                                                ...
                                                <select name="products[0][id]"
                                                    ...
                                                    @foreach($products as $product)
                                                        <option value="{{ $product->id }}">
                                                            {{ $product->sku }} - {{ $product->name }} (Current:
                                ...
                                @if($errors->has('products'))
                                    <p class="text-red-400 text-xs p-4 bg-red-900/20">Please add at least one item to order.
                                    </p>
                                @endif
                            </div>
                            ...
                            {{-- Notes --}}
                            <div class="mb-8">
                                <x-input-label for="notes" :value="__('Notes for Supplier')"
                                    class="font-bold !text-gray-300" />
                                <textarea id="notes" name="notes"
                                    ...
                                    rows="2"
                                    placeholder="e.g. Please ship with urgent priority...">{{ old('notes') }}</textarea>
                            </div>

                            {{-- Footer --}}
                            <div class="flex justify-end gap-4">
                                <button type="submit"
                                    class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow-lg shadow-blue-500/20 transition transform hover:-translate-y-0.5">
                                    Send Purchase Order
                                </button>
                            </div>
                        </form>
                ...
                <tr id="row_${rowCount}">
                    <td class="px-4 py-3">
                        <select name="products[${rowCount}][id]" class="w-full rounded-lg border-[#4A5568] bg-[#2D3748] text-white text-sm focus:border-blue-500 focus:ring-blue-500" required>
                            <option value="">-- Select Component --</option>
                            @foreach($products as $product)
                                <option value="{{ $product->id }}">{{ $product->sku }} - {{ $product->name }} (Current: {{ $product->stock_current }})</option>
                            @endforeach
                        </select>
                    </td>
                    <td class="px-4 py-3">
