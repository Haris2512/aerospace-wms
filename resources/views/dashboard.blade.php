            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-sm text-gray-400 mt-1">Welcome back, <span class="text-blue-400 font-bold">{{ Auth::user()->name }}</span> ({{ ucfirst(Auth::user()->role) }})</p>
                    </div>
                    <div class="hidden md:block">
                        <span class="px-3 py-1 rounded-full bg-[#0B1120] border border-[#2D3748] text-xs text-gray-500 font-mono">
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                @if(in_array(Auth::user()->role, ['admin', 'manager']))
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                        {{-- Card 1 --}}
                        <div class="bg-[#151B2D] p-6 rounded-2xl border border-[#2D3748] shadow-xl relative overflow-hidden group">
                            <div class="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition">
                                <svg class="w-16 h-16 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>
                            </div>
                        ...
                        <div class="bg-[#151B2D] p-6 rounded-2xl border border-[#2D3748] shadow-xl relative overflow-hidden group">
                            ...
                            <p class="text-xl font-black text-green-400 mt-2">Rp {{ number_format($data['total_value'], 0, ',', '.') }}</p>
                        </div>

                        {{-- Card 3 (Alert) --}}
                        <div class="bg-[#151B2D] p-6 rounded-2xl border border-red-900/50 shadow-xl relative overflow-hidden group">
                            <div class="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition">
                            ...
                            <p class="text-3xl font-black text-red-500 mt-2">{{ $data['low_stock_count'] }} <span class="text-sm font-normal text-red-300">Items</span></p>
                        </div>
                        ...
                        <div class="bg-[#151B2D] p-6 rounded-2xl border border-[#2D3748] shadow-xl relative overflow-hidden group">
                            ...
                            <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">Pending Transactions</p>
                            <p class="text-3xl font-black text-yellow-400 mt-2">{{ $data['pending_transactions'] }}</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div class="bg-[#151B2D] rounded-2xl border border-[#2D3748] overflow-hidden shadow-xl">
                            <div class="px-6 py-4 border-b border-[#2D3748] flex justify-between items-center bg-[#1A202C]">
                                ...
                                <a href="{{ route('products.index') }}" class="text-xs text-blue-400 hover:underline">View All</a>
                            </div>
                                ...
                                <tbody class="divide-y divide-[#2D3748]">
                                    @forelse($data['low_stock_items'] as $item)
                                        <tr class="hover:bg-[#1F2937] transition">
                                            ...
                                            <td class="px-6 py-4 text-center text-red-400 font-bold">{{ $item->stock_current }}</td>
                                            <td class="px-6 py-4 text-center text-gray-500">{{ $item->stock_minimum }}</td>
                                            <td class="px-6 py-4 text-right">
                                                @if(Auth::user()->role === 'manager')
                                                    <a href="{{ route('restock.create') }}" class="text-blue-400 hover:text-blue-300 text-xs">Restock</a>
                                                @endif
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="4" class="px-6 py-4 text-center text-gray-500">Stock levels are healthy.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                        ...
                        <div class="bg-[#151B2D] rounded-2xl border border-[#2D3748] p-6 shadow-xl">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <a href="{{ route('products.create') }}" class="flex flex-col items-center justify-center p-4 bg-[#1F2937] rounded-xl border border-[#374151] hover:border-blue-500 hover:bg-[#2D3748] transition group">
                                    <svg class="w-8 h-8 text-blue-500 mb-2 group-hover:scale-110 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                                    <span class="text-sm font-bold text-gray-300 group-hover:text-white">Add Component</span>
                                </a>
                                @if(Auth::user()->role === 'manager')
                                    <a href="{{ route('restock.create') }}" class="flex flex-col items-center justify-center p-4 bg-[#1F2937] rounded-xl border border-[#374151] hover:border-green-500 hover:bg-[#2D3748] transition group">
                                        <svg class="w-8 h-8 text-green-500 mb-2 group-hover:scale-110 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path></svg>
                                        <span class="text-sm font-bold text-gray-300 group-hover:text-white">New Restock PO</span>
                                    </a>
                                @endif
                            </div>
                        </div>
                ...
                @if(Auth::user()->role === 'staff')
                    ...
                    <div class="bg-[#151B2D] rounded-2xl border border-[#2D3748] overflow-hidden shadow-xl">
                        <div class="px-6 py-4 border-b border-[#2D3748] bg-[#1A202C] flex justify-between items-center">
                            ...
                            <a href="{{ route('transactions.create') }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-lg transition">
                                + New Transaction
                            </a>
                        </div>
                        <table class="w-full text-sm text-left">
                            ...
                            <tbody class="divide-y divide-[#2D3748]">
                                @forelse($data['incoming_orders'] as $order)
                                    <tr class="hover:bg-[#1F2937] transition">
                                        ...
                                        <td class="px-6 py-4 text-gray-400">{{ \Carbon\Carbon::parse($order->order_date)->format('d M Y') }}</td>
                                        <td class="px-6 py-4 text-blue-400">{{ $order->products->count() }} Items</td>
                                        <td class="px-6 py-4">
                                            <a href="{{ route('restock.show', $order) }}" class="text-xs bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded">View</a>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
